...
BASE_URL="${1:-http://localhost:8002/api/v1}"
PASS=0
FAIL=0
...
echo "============================================"
echo "  Python Multi-Tenant Auth — Curl Test Suite"
...
echo ""
echo "--- RBAC: Assign super_admin + test admin endpoints ---"
...
CREATE=$(curl -s -X POST "$BASE_URL/tenants" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"name":"Test Corp","slug":"test-corp"}')
TENANT_ID=$(echo "$CREATE" | python3 -c "import sys,json; print(json.load(sys.stdin)['data']['id'])")
[ -n "$TENANT_ID" ] && pass "POST /tenants → created" || fail "POST /tenants" "$CREATE"

LIST=$(curl -s "$BASE_URL/tenants" -H "Authorization: Bearer $TOKEN")
echo "$LIST" | python3 -c "import sys,json; assert len(json.load(sys.stdin)['data']) >= 1" && pass "GET /tenants" || fail "GET /tenants" "$LIST"
...
MEMBER_ROLE_ID=$(docker exec $(docker ps --filter "publish=8002" --format "{{.Names}}" | head -1) psql -U postgres -d authdb -t -c "SELECT id FROM roles WHERE name='tenant_member';" | tr -d ' \n')
...
echo "$ACCEPT" | python3 -c "import sys,json; assert 'accepted' in json.load(sys.stdin)['data']['message'].lower()" && pass "POST /tenants/:id/invitations/accept" || fail "POST accept invitation" "$ACCEPT"
